...
Utility = {
	...
	multiselect: function (start, end) {
		...
		Session.set('menu.x', end.x + end.width);
		Session.set('menu.y', end.y + end.height);
	},
	...
	emptyNeighbours: function () {
		Squares.find({
			x: {
				$lte: Grid.startSelect.x+Grid.startSelect.width,
				$gte: Grid.startSelect.x-1
			},
			y: {
				$lte: Grid.startSelect.y+Grid.startSelect.height,
				$gte: Grid.startSelect.y-1
			},
			value: {
				$exists: false
			}
		}).forEach(function (doc) {
			Squares.update(doc._id,{
				$set: {
					selected:true
				}
			});
		...
		moveCursor: function(direction, shiftKey) {
			...
			if (newSquare) {
				...
				if (shiftKey) {
					...
					Utility.multiselect(origin? origin: Grid.startSelect, newSquare);
				...
				//Set menu position to middle of tile
